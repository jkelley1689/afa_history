...
import { getPassingStat } from "../graphql/queries";
import { updatePassingStat } from "../graphql/mutations";
const client = generateClient();
export default function PassingStatUpdateForm(props) {
  const {
    id: idProp,
    passingStat: passingStatModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = passingStatRecord
      ? { ...initialValues, ...passingStatRecord }
      : initialValues;
    setCmp(cleanValues.cmp);
    setAtt(cleanValues.att);
    setYds(cleanValues.yds);
    setPct(cleanValues.pct);
    setYpa(cleanValues.ypa);
    setSack(cleanValues.sack);
  ...
  const [passingStatRecord, setPassingStatRecord] =
    React.useState(passingStatModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getPassingStat.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getPassingStat
        : passingStatModelProp;
      setPassingStatRecord(record);
    };
    queryData();
  }, [idProp, passingStatModelProp]);
  React.useEffect(resetStateValues, [passingStatRecord]);
  const validations = {
    cmp: [],
    att: [],
    yds: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updatePassingStat.replaceAll("__typename", ""),
            variables: {
              input: {
                id: passingStatRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "PassingStatUpdateForm")}
      {...rest}
    >
      <TextField
        label="Cmp"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || passingStatModelProp)}
