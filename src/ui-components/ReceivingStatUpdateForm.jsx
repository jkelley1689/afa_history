...
import { getReceivingStat } from "../graphql/queries";
import { updateReceivingStat } from "../graphql/mutations";
const client = generateClient();
export default function ReceivingStatUpdateForm(props) {
  const {
    id: idProp,
    receivingStat: receivingStatModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = receivingStatRecord
      ? { ...initialValues, ...receivingStatRecord }
      : initialValues;
    setRec(cleanValues.rec);
    setYds(cleanValues.yds);
    setAvg(cleanValues.avg);
    setLong(cleanValues.long);
    setTd(cleanValues.td);
    setDrops(cleanValues.drops);
    setYac(cleanValues.yac);
    setErrors({});
  };
  const [receivingStatRecord, setReceivingStatRecord] = React.useState(
    receivingStatModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getReceivingStat.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getReceivingStat
        : receivingStatModelProp;
      setReceivingStatRecord(record);
    };
    queryData();
  }, [idProp, receivingStatModelProp]);
  React.useEffect(resetStateValues, [receivingStatRecord]);
  const validations = {
    rec: [],
    yds: [],
    avg: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateReceivingStat.replaceAll("__typename", ""),
            variables: {
              input: {
                id: receivingStatRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ReceivingStatUpdateForm")}
      {...rest}
    >
      <TextField
        label="Rec"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || receivingStatModelProp)}
